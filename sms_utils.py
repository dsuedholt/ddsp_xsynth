...
# These functions are adapted from the sms-tools library for Spectral Modeling Synthesis:
# https://github.com/MTG/sms-tools/blob/master/software/models/utilFunctions.py

# Modifications to the original code:
# - replaced numpy calls with torch calls
# - removed phase calculations, not needed for this application


def detect_peaks(mX, t=torch.tensor(-95.0, dtype=torch.float32)):
    """
...
def detect_harmonics(
    ...
    hfreqp: torch.Tensor,
    fs: int,
    harmDevSlope=torch.tensor(0.01, dtype=torch.float32),
):
    """
